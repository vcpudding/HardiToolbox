...
long double bessi0(long double x, bool bNormalize = false)
{
  long double ax,ans;
  ...
  if ((ax=fabs(x)) <3.75) { //Polynomial fit.
    ...
    y=3.75/ax;
    if (bNormalize)
      ans=(1.0/sqrt(ax))*(0.39894228+y*(0.1328592e-1+y*(0.225319e-2+y*(-0.157565e-2+y*(0.916281e-2+y*(-0.2057706e-1+y*(0.2635537e-1+y*(-0.1647633e-1+y*0.392377e-2))))))));
    else
      ans=(expl(ax)/sqrt(ax))*(0.39894228+y*(0.1328592e-1+y*(0.225319e-2+y*(-0.157565e-2+y*(0.916281e-2+y*(-0.2057706e-1+y*(0.2635537e-1+y*(-0.1647633e-1+y*0.392377e-2))))))));
  }
  return ans;
}
/*
 ...
 * for any real x.
 */
long double bessi1(long double x, bool bNormalize = false)
{
	long double ax,ans;
