function Snew=CreateSignal(x,g_, order, delta)
%-fanDTasia ToolBox------------------------------------------------------------------
...
for k=1:length(g_)
    ...
    for i=0:order
        for j=0:order-i
            Bg(k,c)=0;
            for integr=1:321
                Bg(k,c)=Bg(k,c)+x(c)*exp(-delta*(g2(integr,:)*g_(k,:)')^2)*(g2(integr,1)^i)*(g2(integr,2)^j)*(g2(integr,3)^(order-i-j));
            end
            Bg(k,c)=Bg(k,c)/321; %*4*pi/642;
            c=c+1;
        end
    end
end
