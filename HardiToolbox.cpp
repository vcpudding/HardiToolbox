...
mat HardiToolbox::simulateBallAndStickByComponent(int bVal, double s0, const vec &d, const vec &w, const mat &gradientOrientations, const mat &fibDirs)
{
  ...
  for (int i=0; i<nGrads;++i)
  {
    dwSignal(i, nFibers) = (w.is_empty()?1.0:w(nFibers))*s0*exp(-bVal*d(nFibers));
  }
...
{  
  ...
  if (nFibers>1) {
    cout <<acos(fabs(dot(fibDirs.col(0), fibDirs.col(1))))*180/M_PI <<endl;
  }

  // fibDirs <<0.7634 <<0.6458 <<0.0151 <<endr /*<<0 <<1 <<0 <<endr*/;
  // fibDirs = fibDirs.t();
  ...
  for (int it=0; it<options.maxIt; ++it)
  {
	...
	for (int i=0; i<nGrads; ++i) {
	  ...
	  // cout <<(estimatedSignalSum(i) - dwSignal(i)*bessiRatio(x)) <<endl;
	}
...
void HardiToolbox::estimateTensor (FiberComposition &fibComp, const vec &dwSignal, const mat &gradientOrientations, int bVal, double s0)
{
  ...
  fibComp.fibDiffs <<eigVal(2) <<endr <<(eigVal(0)+eigVal(1))/2.0;
